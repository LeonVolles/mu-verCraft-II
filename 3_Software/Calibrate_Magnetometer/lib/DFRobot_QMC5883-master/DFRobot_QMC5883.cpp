...
bool DFRobot_QMC5883::begin(void)
{
  ...
  if (ICType == IC_NONE)
  {
    ...
    {
      // IMPORTANT (ESP32): calling begin() without pins can reset Wire to default pins.
      // This project initializes Wire with custom SDA/SCL pins in the sketch.
      // If SDA_PIN/SCL_PIN are provided at build time, prefer them here.
#if defined(ARDUINO_ARCH_ESP32) && defined(SDA_PIN) && defined(SCL_PIN)
      _pWire->begin((int)SDA_PIN, (int)SCL_PIN);
#else
      _pWire->begin();
#endif
